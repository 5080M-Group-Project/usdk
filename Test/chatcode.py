...
Q = np.diag([2000, 1, 180, 0]) # Penalties for pitch error, pitch rate, velocity error, and yaw rate error
R = np.diag([0.1, 0.1])
P = solve_continuous_are(A, B, Q, R)
K = np.linalg.inv(R) @ B.T @ P

base_pitch_offset = 0.05 # decreasing makes drift more backward, increasing causes foward drift
...
try:
    while True:
        ...
        left_cmd = control[0] - 0.5*control[1]
        right_cmd = control[0] + 0.5*control[1]
        left_cmd = np.clip(left_cmd, -max_wheel_speed, max_wheel_speed)
        right_cmd = np.clip(right_cmd, -max_wheel_speed, max_wheel_speed)
